...
AddEventHandler('esx_poolcleaner:GiveItem', function()
	...
	if Quantity >= 15 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Vous avez Nettoyer la picine.')
	end
end
)
	...
	SetTimeout(Config.Zones.Sale.ItemTime, function()
		...
		if PlayersSale[source] == true then
			...
			if Quantity < Config.Zones.Sale.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, 'Vous n\'avez plus de   '..Config.Zones.Sale.ItemRequires_name..'s a vendre.')
				...
				TriggerClientEvent('esx:showNotification', _source, 'Vous avez reçus ~g~$' .. Config.Zones.Sale.ItemPrice)
				Sale(_source)
			end
...
AddEventHandler('esx_poolcleaner:startSale', function()
	...
	if PlayersSale[_source] == false then
		...
		TriggerClientEvent('esx:showNotification', _source, 'TMerci, vous avez été payer pour votre travail')
		Sale(_source)
	end
end)
