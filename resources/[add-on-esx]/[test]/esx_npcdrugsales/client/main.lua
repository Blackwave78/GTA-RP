          ...
          if IsPedInAnyVehicle(ped) == false then
            ...
            if pedType ~= 28 and IsPedAPlayer(ped) == false then
              ...
              if distance <= 2 and ped  ~= GetPlayerPed(-1) and ped ~= oldped then
                ...
                if has == true then
                 -- drawTxt(0.90, 1.40, 1.0,1.0,0.4, "Press ~g~E ~w~to attempt a drug deal...", 255, 255, 255, 255)
                 ESX.ShowHelpNotification('Appuyez sur ~ g ~ ~INPUT_CONTEXT~ ~ w ~ pour tenter de vendre de la drogue ...') 
...
AddEventHandler('notifyc', function()
    ...
    if crossing ~= nil then
      ...
      TriggerServerEvent('esx_phone:send', "police", "Une personne louche vend de la drogue " .. streetName .. " and " .. crossing, true, {
        x = coords.x,
        y = coords.y,
        z = coords.z
      })
    else
      TriggerServerEvent('esx_phone:send', "police", "Une personne louche vend de la drogue " .. streetName, true, {
        x = coords.x,
        y = coords.y,
        z = coords.z
      })
