  ...
  AddEventHandler('drugs:trigger', function()
	...
	selling = true
	    if selling == true then
			...
			xPlayer.showNotification('Vente en cours ...')
  ...
  AddEventHandler('drugs:sell', function()
	...
	local paymento = math.random (150,300)
		...
		if coke >= 1 and success == true then
			xPlayer.showNotification('Vous avez vendu de la cocaïne pour $' .. paymentc)
  		...
  		elseif weed >= 1 and success == true then
			xPlayer.showNotification('Vous avez vendu de l\'herbe pour $' .. paymentw)
		--		TriggerClientEvent("pNotify:SetQueueMax", source, "lmao", 5)
  		--		TriggerClientEvent("pNotify:SendNotification", source, {
		...
		--	})
			...
			xPlayer.showNotification('Vous avez vendu de la Meth pour $' .. paymentm)
...
--			})
			...
			--	xPlayer.showNotification('You have sold some Opium for $' .. paymento)
	...
	--		xPlayer.removeInventoryItem('opium_pooch', 1)
  	...
  	--		selling = false
			elseif selling == true and success == false and notintrested == true then
				...
				xPlayer.showNotification('Le client n\'est pas interéssé !' )
	...
	--		})
  			...
  			elseif meth < 1 and coke < 1 and weed < 1 and opium < 1 then
				xPlayer.showNotification('Vous n\'avez pas de drogue')
	...
	--		})
			elseif copscalled == true and success == false then
				xPlayer.showNotification('s\'est quoi cette merde !! ,j\'apelle la police')
...
AddEventHandler('sell_dis', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.showNotification('Tu t\'es trop éloigné de la personne')
--	TriggerClientEvent("pNotify:SetQueueMax", source, "lmao", 5)
--		TriggerClientEvent("pNotify:SendNotification", source, {
...
AddEventHandler('checkD', function()
	...
	if meth >= 1 or coke >= 1 or weed >= 1 then
