...
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
		RageUI.Button(_U('vehicle_gps_button'), nil, {}, true, function(Hovered, Active, Selected, source)
			if (Selected) then
				...
				-- if GetEntityModel(Vehicle) == -1848994066 then
					 if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
						 ...
						 while not hasArrived do
							 ...
							 if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
								 drawTxt('Autopilot: ~g~On', 0.07, 0.24, 0.4)
								 drawTxt('limitation : ' .. speed .. 'KM/H', 0.07, 0.28, 0.4)
								 drawTxt('accelerer: ↑', 0.07, 0.32, 0.4)
								 drawTxt('ralentir: ↓', 0.07, 0.36, 0.4)
								 drawTxt('Stop: X', 0.07, 0.4, 0.4)
								 drawTxt('Distance: ' .. math.ceil(GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, false)) .. 'M', 0.07, 0.44, 0.4)
								 ...
								 if IsControlJustPressed(0, 173) then
									 if speed >= 6 then
										 local Slower = speed-5
										 speed = Slower
										 SetEntityMaxSpeed(Vehicle, speed/3.6)
									 end
								 elseif IsControlJustPressed(0, 27) then
									 if speed <= 200 then
										 local Faster = speed+5
										 speed = Faster
										 SetEntityMaxSpeed(Vehicle, speed/3.6)
									 end
								 end
...
function drawTxt(text, x, y, scale)
    ...
    DrawText(x, y)
    DrawRect(0.0, 0.36, 0.3, 0.25, 153, 0, 0, 30) --x,y,x
end
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
