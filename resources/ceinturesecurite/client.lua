	...
	while true do
		...
		if car ~= 0 and (wasInCar or IsCar(car)) then
			...
			if IsControlJustReleased(0, 73) then
				...
				TriggerEvent("ls:notify", "~g~Vous avez mis votre Ceinture", 0.200)
				--TriggerEvent('esx:showAdvancedNotification', '~g~Vous avez mis votre Ceinture')
				else 
				TriggerEvent("ls:notify", "~r~Vous avez retiré votre Ceinture", 0.200)
				--TriggerEvent('esx:showAdvancedNotification', '~r~Vous avez retiré votre Ceinture')
				 end 
            end
...
AddEventHandler('ol:ceinture', function()
	...
	if car ~= 0 then
		beltOn = not beltOn				  
		if beltOn then 
			TriggerEvent('esx:showAdvancedNotification', '~r~Vous avez mis votre Ceinture')
		else
		 	TriggerEvent('esx:showAdvancedNotification', '~r~Vous avez retiré votre Ceinture')
		end 
	end
