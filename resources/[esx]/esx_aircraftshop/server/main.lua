...
function RemoveOwnedVehicle(plate)
	MySQL.Async.execute('DELETE FROM owned_vehicles_plane WHERE plate = @plate', {
		['@plate'] = plate
	})
end
...
AddEventHandler('esx_aircraftshop:setVehicleOwned', function(vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles_plane (owner, plate, vehicle, Type) VALUES (@owner, @plate, @vehicle, @Type)',
	{
...
ESX.RegisterServerCallback('esx_aircraftshop:isPlateTaken', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles_plane WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
		cb(result[1] ~= nil)
	end)
...
AddEventHandler('esx_aircraftshop:setJobVehicleState', function(plate, state)
	...
	MySQL.Async.execute('UPDATE owned_vehicles_plane SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		['@stored'] = state,
		['@plate'] = plate,
		['@job'] = xPlayer.job.name
	}, function(rowsChanged)
