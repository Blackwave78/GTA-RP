...
function SpawnCar()
	ESX.TriggerServerCallback('esx_carthief:isActive', function(isActive)
		if isActive == 0 then
			...
			ESX.ShowNotification("Il y a deja une livraison en cours ...")
		end
	end)
end
...
function FinishDelivery()
  if(GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) and GetEntitySpeed(car) < 3 then
    ...
    TriggerServerEvent('esx_carthief:stopalertcops')
		...
		TriggerEvent('esx:showNotification', "Vous devez utiliser la voiture qui vous a été fournie et vous devez vous arrêter complètement.")
  end
end
  ...
  while true do
    Wait(1000)
		if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) then
			TriggerEvent('esx:showNotification', "Vous avez 1 minute pour revenir dans la voiture")
			Wait(50000)
			if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) then
				TriggerEvent('esx:showNotification', "Vous avez 10 secondes pour revenir dans la voiture")
				Wait(10000)
				TriggerEvent('esx:showNotification', "Mission échoué!")
				AbortDelivery()
			end
		end
	end
...
AddEventHandler('esx_carthief:setcopnotification', function()
	ESX.ShowNotification("Voiture en train de voler. Tracker de véhicule sera actif sur votre radar")
end)

AddEventHandler('esx_carthief:hasEnteredMarker', function(zone)
  if LastZone == 'menucarthief' then
    CurrentAction     = 'carthief_menu'
    CurrentActionMsg  = 'Appuyer sur ~INPUT_CONTEXT~ Pour recuperer une voiture volé a livré'
    CurrentActionData = {zone = zone}
  elseif LastZone == 'cardelivered' then
    CurrentAction     = 'cardelivered_menu'
    CurrentActionMsg  = 'Appuyer sur ~INPUT_CONTEXT~ pour deposer le vehicule'
    CurrentActionData = {zone = zone}
  end
end)
