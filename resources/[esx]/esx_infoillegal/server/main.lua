...
RegisterServerEvent('esx_infoillegal:WMoney')
AddEventHandler('esx_infoillegal:WMoney', function()
	local source = source
	TriggerEvent('es:getPlayerFromId', source, function(user)
		if (tonumber(user.getMoney()) >= Config.PriceWashBM) then
			user.removeMoney(Config.PriceWashBM)
			TriggerClientEvent("esx_infoillegal:WashMoney", source)			
		else
			TriggerClientEvent("esx_infoillegal:notify", source, "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('NoCash'))
		end
	end)
...
RegisterServerEvent('esx_infoillegal:Bmarker')
AddEventHandler('esx_infoillegal:Bmarker', function()
	local source = source
	TriggerEvent('es:getPlayerFromId', source, function(user)
		if (tonumber(user.getMoney()) >= Config.BlackweashopPR) then
			user.removeMoney(Config.BlackweashopPR)
			TriggerClientEvent("esx_infoillegal:Blackweashop", source)			
		else
			TriggerClientEvent("esx_infoillegal:notify", source, "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('NoCash'))
		end
	end)
...
RegisterServerEvent('esx_infoillegal:BCmarker')
AddEventHandler('esx_infoillegal:Bmarker', function()
	local source = source
	TriggerEvent('es:getPlayerFromId', source, function(user)
		if (tonumber(user.getMoney()) >= Config.BlackweashopPR) then
