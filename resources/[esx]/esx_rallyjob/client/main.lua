...
local zOffset = -0.65	
local hintToDisplay = "pas d\'indice"			
...
function refreshBlips()
	if playerData.job.name ~= nil and playerData.job.name == 'rally' then 
		drawBlip(Config.locker, 366, "Race local ")					
		drawBlip(Config.carSpawn, 315, "Spwancar")					
		drawBlip(Config.carDel, 490, "Delcar")					
	end
end
...
function spawnCar()
	Citizen.CreateThread(function()
		...
		SetVehicleNumberPlateText(currentCar, "Race" .. math.random(1000, 9999))									
...
function startWork(flag)
	...
	if IsPedInAnyVehicle(playerPed) and isMyCar() then 
		...
		if elementAt(taskPoints, currentIndex) == nil then
			ESX.ShowNotification('~w~Temps de course ~b~' .. lastRoute .. ' ~w~tu l\'as eu ~y~' .. msToClock(stopWatch:SaveTime('lap')))
			chat("6", "tour: ^4" .. msToClock(stopWatch:GetSavedTime('lap')))
			Citizen.CreateThread(function()
			...
			TriggerEvent('esx:showNotification', "Instruction : ~b~retourner au depart~g~")
		else
			if flag then
		...
		ESX.ShowNotification('~r~VOtre voiture est')	
	end
end
...
function giveWork()
	...
	spawnCar()
	ESX.ShowNotification("Caution ~r~" .. Config.caution .. "~g~ € ~w~vous on été prélevé.")
	TriggerServerEvent('esx_rallyjob:getPunished', Config.caution)
	setGps(elementAt(taskPoints, currentIndex))									     
	ESX.ShowNotification("~w~Aller a ~b~ la ligne de depart .") --alert player
end
...
function getPayAmount()
	...
	if stopWatch:GetSavedTime('lap') < trackinfo.gold then
		...
		ESX.ShowNotification('~w~Vous avez la ~y~Médaille d\'or!')
	...
	elseif stopWatch:GetSavedTime('lap') < trackinfo.silver then
		...
		ESX.ShowNotification('~w~Vous avez la  ~y~Médaille d\'argent!')
		TriggerEvent('esx_rallyjob:celebrate')
	elseif stopWatch:GetSavedTime('lap') < trackinfo.bronze then
		...
		ESX.ShowNotification('~w~Vous avez la  ~y~Médaille de bronze!')
		TriggerEvent('esx_rallyjob:celebrate')
	else
	...
	ESX.ShowNotification('~w~Vous avez gagné ~b~' .. amount .. '~g~ €~w~, Comprend les coûts de réparation ~r~' .. penalty .. '~g~ €')
...
function getPaid()
	...
	elseif isMyCar() then
		...
		ESX.ShowNotification('~r~Pas de rallye, pas de salaire. ~w~Un droit d\'entrée a été déduit de la prime, -~r~500~g~€')
	else
		TriggerEvent('esx_rallyjob:ragequit')
		ESX.ShowNotification('Où était la voiture?')
		emeDel()
	end
	currentPlate = ''										
	currentVehicle = nil										
	...
	while true do
		if onDuty and elementAt(taskPoints, currentIndex) ~= nil then
			...
			if currentIndex == 1 and isInside(pCoords, elementAt(taskPoints, currentIndex), 0.9) and isMyCar() then
				...
				FreezeEntityPosition(GetVehiclePedIsIn(GetPlayerPed(-1), false), true)
				ESX.ShowNotification('~w~Ralli ~g~commencer~w~...')
				startWork(false)
				...
				ESX.TriggerServerCallback('esx_rallyjob:getTimes', function(times)
					...
					chat("7", "L'or ^3@^9" .. trackinfo.driver)
					Citizen.Wait(3000)
	...
	while true do																
		...
		if not menuIsOpen then
			...
			elseif onDuty and elementAt(taskPoints, currentIndex) == nil and isInside(pCoords, Config.carSpawn, 1.5) then
				isInMarker = true
				displayHint = true
				hintToDisplay = "Appuyez sur E pour racheter la voiture de rallye"
				currentZone = 'start'
			elseif playerData.job ~= nil and playerData.job.name == "rally" and currentPlate ~= '' and isInside(pCoords, Config.carDel, 2.5) then  				
				isInMarker = true
				displayHint = true
				hintToDisplay = "Appuyez sur E pour retourner la voiture de rallye"
				currentZone = 'pay'
			else																				
				isInMarker = false
				displayHint = false
				hintToDisplay = "Aucune indication à afficher"
				currentZone = 'none'
			end
...
function openMenu()									
  ...
  ESX.UI.Menu.Open(
    ...
    {
      title    = "Local de Rallye",							
      elements = {
        {label = "Vêtements de Rallye", value = 'rally_wear'},		
        {label = "Vétements personelle", value = 'everyday_wear'}	
      }
    },
    function(data, menu)									
