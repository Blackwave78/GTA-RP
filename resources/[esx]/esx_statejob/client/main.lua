...
function OpenCloakroomMenu()
  ...
  if Config.EnableNonFreemodePeds then
      table.insert(elements, {label = _U('state_wear'), value = 'state_wear'})
    table.insert(elements, {label = _U('state_wear'), value = 'state_wear'})
    table.insert(elements, {label = _U('state_wear'), value = 'state_wear'})
  end
...
Citizen.CreateThread(function()
  ...
  while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'state' then
				...
				if(IsControlJustPressed(1, 38)) then
					TriggerServerEvent("esx_statejob:sendSonnette")
				end
			end
		end
...
RegisterNetEvent("esx_statejob:sendRequest")
AddEventHandler("esx_statejob:sendRequest", function(name,id)
	stopRequest = true
	...
	while not stopRequest do
		...
		if(IsControlJustPressed(1, 23)) then
			TriggerServerEvent("esx_statejob:sendStatusToPoeple", id, 1)
			stopRequest = true
		end

		if(IsControlJustPressed(1, 47)) then
			TriggerServerEvent("esx_statejob:sendStatusToPoeple", id,0)
			stopRequest = true
		end
	end
end)

RegisterNetEvent("esx_statejob:sendStatus")
...
Citizen.CreateThread(function()
  while true do
   ...
   if IsControlPressed(0,  Keys['=']) and PlayerData.job ~= nil and PlayerData.job.name == 'state' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'state_actions') and (GetGameTimer() - GUI.Time) > 150 then
