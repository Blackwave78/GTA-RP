...
AddEventHandler('esx_handcuffs:cuffcheck', function()
  ...
  if distance ~= -1 and distance <= 3.0 then
				  ...
				  TaskPlayAnim(ped,"amb@prop_human_bum_bin@idle_b","idle_d",100.0, 200.0, 0.3, 120, 0.2, 0, 0, 0, 130)
					ESX.ShowNotification('~g~Vous avez Utilisé des menottes')
				Wait(8000)
		TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(player))
				ESX.ShowNotification('~r~Player menotté/démenotté ...')
  else
    ESX.ShowNotification('Pas de joeurs a proximité')
	end
end)
...
AddEventHandler('esx_handcuffs:unlockingcuffs', function()
  local player, distance = ESX.Game.GetClosestPlayer()
	...
	if IsLockpicking == false then
		...
		ESX.ShowNotification('Vous étes déja entrain de crochetté les menottes')
	end
end)
