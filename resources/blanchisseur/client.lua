...
local showBlip = false -- Show blip on map // afficher le point sur la carte ||||||||| False to disable
local maxDirty = 1000000 -- Player allowed to laundering per 100 000 // Le joueur ne peut blanchir que 100 000$ par 100 000$
...
function Blanchir(amount)
		...
		if(amount ~= -1) then
			local amounttime = amount * 2 
			if(amount <= 250) then
				exports['progressBars']:startUI(1000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 500) then
				exports['progressBars']:startUI(2500, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 1000) then
				exports['progressBars']:startUI(3500, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 2000) then
				exports['progressBars']:startUI(4000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 3000) then
				exports['progressBars']:startUI(5000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 4000) then
				exports['progressBars']:startUI(6000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 5000) then
				exports['progressBars']:startUI(7000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 6000) then
				exports['progressBars']:startUI(8000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			...
			elseif(amount >= 9000) then
				exports['progressBars']:startUI(9000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 10000) then
				exports['progressBars']:startUI(10000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 15000) then
				exports['progressBars']:startUI(15000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 20000) then
				exports['progressBars']:startUI(18000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
			elseif(amount >= 80000) then
				exports['progressBars']:startUI(19000, "Wash")
				TriggerServerEvent("blanchisseur:BlanchirCash", amount, amounttime)
