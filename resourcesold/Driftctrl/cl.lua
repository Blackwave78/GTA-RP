Citizen.CreateThread( function()
	...
	if IsPedInAnyVehicle(ped) then	
			...
			--110 numpad 5
				if IsControlJustReleased(1, 110) then -- and ((GetVehicleClass(vehicle) == 0) or (GetVehicleClass(vehicle) == 1) or (GetVehicleClass(vehicle) == 2) or (GetVehicleClass(vehicle) == 3) or (GetVehicleClass(vehicle) == 4) or (GetVehicleClass(vehicle) == 5) or (GetVehicleClass(vehicle) == 6) or (GetVehicleClass(vehicle) == 7) or (GetVehicleClass(vehicle) == 9)) then
...
function DriftOff()
				...
				local currentEngineMod = GetVehicleMod(vehicle, 11)
					...
					drawNotification('~y~TCS~s~, ~y~ABS~s~, ~y~ESP ~s~=> ~g~Activé~s~!')
					drawNotification('Mode drift desactivé')
...
function DriftOn()
				...
				local addTofLowSpeedTractionLossMult = (GetVehicleHandlingFloat(vehicle, "CHandlingData", "fLowSpeedTractionLossMult")-0.57)
					...
					drawNotification('~y~TCS~s~, ~y~ABS~s~, ~y~ESP ~s~=> ~r~OFF~s~!')
					drawNotification('Mode drift activé!')
