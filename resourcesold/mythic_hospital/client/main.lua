...
--{X: 341.274, Y: -582.904, Z: 28.791} {H: 92.458} {Comment: hospital check},
local hospitalCheckin = { x = 341.274, y = -582.904, z = 28.791, h = 180.4400994227 } --{ x = 308.06161499023, y = -595.19683837891, z = 43.291839599609, h = 180.4409942627 }
local pillboxTeleports = {
    { x = 354.216, y = -562.11, z = 29.718, h = 64.513374328613, text = 'Press ~INPUT_CONTEXT~ ~s~to go to lower Pillbox Entrance' },
...
AddEventHandler('mythic_hospital:client:RPSendToBed', function(id, data)
    ...
    Citizen.CreateThread(function()
        while bedOccupyingData ~= nil do
            Citizen.Wait(1)
            PrintHelpText('Appuyer sur ~INPUT_VEH_DUCK~ pour vous lever')
...
AddEventHandler('mythic_hospital:client:SendToBed', function(id, data)
    ...
    Citizen.CreateThread(function ()
        ...
        exports['mythic_notify']:DoHudText('inform', 'Les médecins soigne vos blessure')
...
AddEventHandler('mythic_hospital:client:FinishServices', function()
    ...
    exports['mythic_notify']:DoHudText('inform', 'Vous étes maintenant soigné')
    LeaveBed()
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 10 then
            DrawMarker(27, hospitalCheckin.x, hospitalCheckin.y, hospitalCheckin.z - 1.5, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 1, 157, 0, 155, false, false, 2, false, false, false, false)

            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                if distance < 3 then
                    PrintHelpText('Appuyer sur ~INPUT_CONTEXT~ ~s~ pour faire un check-up ')
                    if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 200) or (IsInjuredOrBleeding()) then
                            ...
                            exports['mythic_notify']:DoHudText('error', 'Vous n\'avez pas besoin d\'étes soigné !')
                        end
                    end
                end
            end
