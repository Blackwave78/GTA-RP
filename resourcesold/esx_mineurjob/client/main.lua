-----------------------------------------
-- Created and modify by New-WaveRp
-- Bl@ckw@ve
-----------------------------------------
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'biff'}
			}
			if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss' then
				table.insert(elements, {label = 'Vehicule patron', value = 'mesa'})
			end
...
--[[
			if v.Type == 2 then
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString("Entreprise Mineur")
				EndTextCommandSetBlipName(blip2)
				table.insert(JobBlips, blip2)
			end
	]]
...
--	end
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
		...
		for k,v in pairs(Config.Zones)do
			...
			if v.Type == 21 then
				local blip2 = AddBlipForCoord(Config.Zones.mineurActions.Pos.x, Config.Zones.mineurActions.Pos.y, Config.Zones.mineurActions.Pos.z)
			--	local blip = AddBlipForCoord(2444.408, 4987.925, 46)
				SetBlipSprite (blip2, 617)
				SetBlipDisplay(blip2, 4)
				SetBlipScale  (blip2, 1.0)
				SetBlipColour (blip2, 22)
				SetBlipAsShortRange(blip2, true)
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString("Entreprise Mineur")
				EndTextCommandSetBlipName(blip2)
