...
AddEventHandler('esx_mineurjob:startHarvest', function(zone)
	...
	if PlayersHarvesting[_source] == false then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Vous être en train de ramasser des ~g~Pierres')  
	...
	if PlayersSelling[source] == true then
		...
		if zone == 'SellFarm' then
			...
			elseif xPlayer.getInventoryItem('or').count <= 0 and jus == 0 then
				...
				tabacsec = 0
				return
			elseif xPlayer.getInventoryItem('diamon').count <= 0 and tabacsec == 0 then
