...
UserBankIDi 			= 1		-- 1 = Maze, 2 = Bank Of Liberty, 3 = Fleeca  (This will show the corresponding message when the player doesn't have enoug money)
AllowEnterTrainWanted	= 0		-- Change to 1 if you want to allow players to ENTER the train when they have a wanted level
TicketPrice				= 5	-- Change to any value YOU think is suitable for a Metro Ticket in your (RP) Server
...
--	{2463.0,3872.0,38.8},
--	{1164.0,6433.0,32.0},
	{537.0,-1324.1,29.1} --tram
--	{219.1,-2487.7,6.0} --tram
}
...
local TicketMachines = {'prop_train_ticket_02', 'v_serv_tu_statio3_'} ----a revoir
...
--		end
	...
	--	Wait(200) -- Added a small 'waiting' while the train is loaded (to prevent the)
		...
		--{X: -1229.122, Y: -2888.399, Z: -8.324}
		MetroTrain = CreateMissionTrain(24,-1229.122,-2888.399,-8.324,true) -- these ones have pre-defined spawns since they are a pain to set up
		...
		Wait(200) -- Added a small 'waiting' while the train is loaded (to prevent the)
				  -- random unexplained spawning of the freight train on the Metro Rails
		--Change location if use two metro !!!!!!!
		--unused for now
		--[[
	...
	while true do
		...
		if IsPlayerNearTicketMachine then
			if not IsPlayerUsingTicketMachine  then
				if not ShowedBuyTicketHelper then
					DisplayHelpText("Appuyer sur ~INPUT_CONTEXT~ pour achetter un ticket de métro  ($" .. TicketPrice .. ")")
					ShowedBuyTicketHelper = true
				end
			else
			...
			if IsControlJustPressed(0, 51) and PlayerHasMetroTicket then	
				SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Touriste Informations", "Vous avez déjà un billet de métro valide, veuillez vous rendre à l’une des stations et monter à bord du métro", true)
			...
			if IsControlJustPressed(0, 51) and not PlayerHasMetroTicket then	
				...
				if XNLUserMoney < TicketPrice then
					...
					SMS_Message(BankIcon, BankName, "Los Santos Transit", "La transaction a échoué, vous n’avez pas suffisamment d\'argent.", true)
				else
					if PayWithBank == 1 then
						local player_id = GetPlayerFromServerId(source)
						-- Put YOUR code to deduct the amount from the players BANK account here
						-- 'Basic Example':  PlayerBankMoney = PlayerBankMoney - TicketPrice
						TriggerServerEvent("XNL:buyItem", player_id)
					...
					SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Informations Touristes ", "Merci pour votre achat, votre billet sera valide pour la session en cours.", true)
			...
			if IsPlayerInMetro then
				if XNLCanPlayerExitTrain() then
					if not XNLTeleportPlayerToNearestMetroExit() then
						SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Informations Touristes", "Désolé, il semble que vous avez pressé un peu trop tard, vous devrez attendre la prochaine station.", true)
					end
					...
					SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Informations Touristes", "Désolée " .. XNLGenMess .. ", mais il n’est pas permis de sortir au hasard du métro. S’il vous plaît attendre la prochaine station!", true)
				end
			end
					...
					if DoesEntityExist(Metro) then
						if GetEntityModel(Metro) == GetHashKey("metrotrain") then
							if not PlayerHasMetroTicket	then
									...
									--==========================================================================
									SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Informations Touristes", "Désolé, vous devrez d’abord acheter un billet de métro LST.", true)
							else
								if IsPlayerWantedLevelGreater(PlayerId(), 0) and AllowEnterTrainWanted == 0 then
									...
									--==========================================================================
									SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Informations Touristes", "Désolé, nous ne permettons pas futugives dans notre métro. Tous les passagers devraient être en mesure de voyager en toute sécurité!", true)
								else
									CurrentMetro = Metro
									...
									PlayerHasMetroTicket = false
									SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Information Touristes ", "Vous êtes entré dans le métro, votre billet a été invalidé.", true)
								end
							end
						else
							IsPlayerNearMetro = false
		...
		if IsPlayerInMetro then
			if ReportTerroristOnMetro == true then
				if GetPlayerWantedLevel(PlayerId()) < 4 then
					if IsPedShooting(GetPlayerPed(-1)) then
						...
						SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Information Touristes", "Nous NE tolérerons PAS le comportement terroriste sur nos véhicules de transport public!", true)
					end
				end
			end
			...
			if not DoesEntityExist(CurrentMetro) then
				...
				IsPlayerInMetro = false
				PlayerHasMetroTicket = true
				SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Tourist Information", "Nos excuses, quelque chose a terriblement mal tourné, vous avez reçu un billet gratuit!", true)
			else
				if IsPlayerInMetro then
					...
					if ShowingExitMetroMessage == true and not ShowedLeaveMetroHelper then
						DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour quitter le métro")
						ShowedLeaveMetroHelper = true
					end
					...
					if GetDistanceBetweenCoords(x,y,z, MetroX, MetroY, MetroZ, true) > 15.0 then
						IsPlayerNearMetro = false
						IsPlayerInMetro = false
						SMS_Message("CHAR_LS_TOURIST_BOARD", "Los Santos Transit", "Informations Tourist", "Merci d’avoir voyagé avec Los Santos Transit.", true)
					end
			...
			if not IsPlayerInVehicle then
				...
				if PlayerHasMetroTicket and XNLCanPlayerExitTrain() then
					if not ShowedEToEnterMetro then
						DisplayHelpText("Appuyer sur ~INPUT_CONTEXT~ en face (et près) du métro pour y entrer.")
