...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			ShowHelpNotif("Appuyez sur ~INPUT_TALK~ pour annuler le plein." .. extraEssence .. extraString)
		else
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				ESX.ShowHelpNotification('Quittez votre véhicule pour mettre le plein.')
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								ESX.ShowHelpNotification('Appuyez sur ~INPUT_TALK~ pour commencer le plein d\'essence.')
								...
								ESX.ShowHelpNotification('Vous n\avez pas assez d\argent.')
							end
						elseif not canFuel then
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.JerryCanEmpty)
						else
							ESX.ShowHelpNotification('Vous avez terminé de mettre le plein d\'essence.')
						end
					end
